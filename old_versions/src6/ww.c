...
{   
    ...
    windowAndRenderer->hwnd = CreateWindowExW(
        0, windowAndRenderer->wc.lpszClassName, windowAndRenderer->windowName, WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN ,
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
    {
        ...
        fprintf(fp, "key down %c\n", (char) instance->key);
        instance->keyboardEvent = WWKEYBOARD_KEYPRESSED;
        instance->keyboardCallback(instance, instance->key, instance->keyboardEvent, instance->keyboardUserData);
        break;
    }
    case WM_KEYUP:
    { 
        // if(instance->keyboardEvent == WWKEYBOARD_KEYRELEASED && instance->key == (wwKey) wParam)
        //     break;
        instance->key = (wwKey) wParam;
        fprintf(fp, "key up %c\n", (char) instance->key);
        instance->keyboardEvent = WWKEYBOARD_KEYRELEASED;
        instance->keyboardCallback(instance, instance->key, instance->keyboardEvent, instance->keyboardUserData);
        break;
    }
    case WM_LBUTTONDOWN:
