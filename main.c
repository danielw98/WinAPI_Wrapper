...
void buttonCallback(wwButton *button, wwWindowAndRenderer *win)
{
    ...
    /* not sure whether the textures should be in the renderer, better not, but keep this 
    so I don't forget that it exists - come back here later */
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wwTimer_Init(&timer);
    fprintf(fp, "About to start main loop\n");
    while(ww_Running(win) == wwTrue)
    {
        fprintf(fp, "Main loop iteration %p\n", win);
        // Render(win, NULL);
        wwTimer_Start(&timer);
